...
def test_get_task_status_not_found(test_client):
    ...
    with patch('app.services.task_manager.task_manager.get_task_status', return_value=None):
...
def test_list_tasks_cleanup(test_client, task_manager, monkeypatch):
    ...
    # Mock the task manager
    with patch('app.services.task_manager.task_manager.tasks', {"old-task-1": old_task}), \
         patch('app.services.task_manager.task_manager.get_task_status', side_effect=lambda x: task_manager.tasks.get(x)), \
         patch('app.services.task_manager.task_manager.cleanup_old_tasks') as mock_cleanup:
